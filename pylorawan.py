...
# This file provides a API to interact with RAK and ASR Wireless Lorawan devices.
# Based on https://github.com/phoughton/pylorawan/blob/main/pylorawan.py - updated and extended for ASR6501
# Tested for RAK3172 on a RAK3272s Breakout Board (868MHz), also for ASR6501 on an M5 LoRaWAN868 
...
# Provide:
    ...
    def __init__(self, uart, device_type, debug=False, uart_debug=None):
        self.uart = uart
        if device_type not in [None, "RAK4200", "RAK3172", "ASR6501"]:
            raise ValueError("Device type has not been set correctly. Supported types: RAK4200, RAK3172, ASR6501")
    ...
    def run_command(self, command=None, wanted_response=None, tries=1, debug=False, rx_delay=4.5):
        ...
        while tries > 0 and not found:
            ...
            if 'EVT' in resp_text or 'CME' in resp_text:
                print(resp_text)
            if not found:
                print("retrying...")
    ...
    def class_translate(self, lora_class):
        ...
        if self.device=="RAK4200":
            ...
            return self.ASR6501_lora_class_lookup[lora_class]
    ...
    def configure_otaa(self, region=None, dev_eui=None, app_eui=None, app_key=None, lora_class=None):
        ...
        if self.device == "RAK4200":
            ...
            self.run_command(command=f"AT+NWM=1", wanted_response="Current Work Mode: LoRaWAN.", tries=1, rx_delay=0.5)
            ...
            self.run_command(command=f"AT+CJOINMODE=0", wanted_response="OK", tries=1, rx_delay=0)
            self.run_command(command=f"AT+CCLASS={lora_class}", wanted_response="OK", tries=1, rx_delay=0)
            self.run_command(command=f"AT+CDEVEUI={dev_eui}", wanted_response="OK", tries=1, rx_delay=0)
            self.run_command(command=f"AT+CAPPEUI={app_eui}", wanted_response="OK", tries=1, rx_delay=0)
            self.run_command(command=f"AT+CAPPKEY={app_key}", wanted_response="OK", tries=1, rx_delay=0)
    ...
    def data_rate(self, data_rate=5):
        if self.device=="RAK4200":
            ...
            raise ValueError("not yet coded for ASR6501")
...
#            print(resp)
            ...
            found, resp =  self.run_command(command='AT+CJOIN=1,0,10,8', wanted_response='+CJOIN:OK', tries=2)
#            print(resp)
        return found
    ...
    def send_data(self, data="", port=1, tries=1):
        if self.device=="RAK4200":
            ...
            length=str(len(data))
            return self.run_command(command=f"AT+DTRX=1,1,{length},{data})", wanted_response="OK+RECV", tries=tries)
#                                            (AT+DTRX=confirm,nbtrials,length,data,)
    ...
    def status_info(self):
        if self.device=="RAK4200":
            ...
            self.log_debug("Status command not supported on ASR6501")
    ...
    def get_supported_regions(self, device_type=None):
        ...
        if device_type == "ASR6501":
            return self.ASR6501_region_list
